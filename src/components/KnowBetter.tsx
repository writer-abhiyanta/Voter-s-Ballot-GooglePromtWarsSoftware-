...
const HISTORICAL_DATA: PastPerformance[] = [
  {
    ...
    details: [
      {
        promise: "Construct 5 new public schools",
        status: "kept",
        workflow: "Funding allocated in 2023, constructed by 2025.",
      },
      {
        promise: "Reduce carbon emissions by 20%",
        status: "in-progress",
        workflow: "Passed green energy bill, currently at 12% reduction.",
      },
      {
        promise: "Zero tax on essential food items",
        status: "broken",
        workflow: "Vetoed by parliament during budget review.",
      },
    ],
  },
  {
    ...
    details: [
      {
        promise: "Increase police funding by 15%",
        status: "kept",
        workflow: "Implemented via budget amendment 4.",
      },
      {
        promise: "Build new commercial district",
        status: "kept",
        workflow: "Zoning approved and phase 1 complete.",
      },
    ],
  },
];
...
/**
 * Know Better - Pre-term Historical Performance Analytical Tool
 *
 * Exposes historical data through a minimal-coupling, master-detail perspective.
 *
 * Algorithmic Complexity: State tracking O(1), Item lookup O(n) handled instantly via client-side caching.
 * Accessibility: Fully operable via keyboard with semantic <section>, <article> structure and ARIA statuses.
 */
export const KnowBetter: React.FC = () => {
  const [selectedHistoricalId, setSelectedHistoricalId] = useState<
    string | null
  >(HISTORICAL_DATA[0].id);
  const activeRecord = HISTORICAL_DATA.find(
    (r) => r.id === selectedHistoricalId,
  );

  return (
    <section
      className="max-w-6xl mx-auto py-8"
      aria-labelledby="know-better-title"
    >
      <header className="mb-8">
        <h2
          id="know-better-title"
          className="text-3xl font-bold text-emerald-400 mb-2 flex items-center gap-3"
        >
          <History className="w-8 h-8" /> Know Better: Pre-Term Performance
        </h2>
        <p className="text-neutral-400">
          Review historical data. Analyze what candidates promised in their
          previous terms, what they actually delivered, and the workflow of
          their actions.
        </p>
      </header>
              ...
              <div className="flex items-center gap-4 text-sm mt-2">
                ...
                <div>
                  ...
                  <span className="font-bold">
                    {record.promisesKept} / {record.promisesMade}
                  </span>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-center justify-between mb-8 pb-6 border-b border-neutral-800">
                <div>
                  ...
                  <span className="bg-neutral-800 text-neutral-300 px-3 py-1 rounded text-sm">
                    {activeRecord.term} Term Audit
                  </span>
                </div>
              ...
              <div className="space-y-6">
                ...
                {activeRecord.details.map((detail, idx) => (
                  <div
                    key={idx}
                    className="bg-neutral-950 border border-neutral-800 rounded-xl p-5 relative overflow-hidden"
                  >
                    <div
                    ...
                    <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-3 text-sm text-neutral-400">
                      <span className="block text-xs font-semibold text-neutral-500 uppercase mb-1">
