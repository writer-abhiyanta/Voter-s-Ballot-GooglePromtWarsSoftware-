...
const Login: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      } else if ((err?.message && err.message.includes("The operation is insecure")) || err?.name === "SecurityError") {
        setError("Authentication blocked by browser settings. Please enable third-party cookies or open the app in a new tab.");
      } else {
        setError(err?.message || "Failed to sign in. Please try again or open in a new tab.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-4 font-sans relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0">
        <motion.div
          ...
          className="absolute -top-40 -left-40 w-96 h-96 bg-emerald-600/20 rounded-full mix-blend-screen filter blur-[100px]"
        ></motion.div>
        <motion.div
          animate={{ scale: [1, 1.2, 1], rotate: [0, -90, 0] }}
          transition={{ repeat: Infinity, duration: 25, ease: "linear" }}
          className="absolute top-40 -right-40 w-96 h-96 bg-green-600/20 rounded-full mix-blend-screen filter blur-[100px]"
        ></motion.div>
        <motion.div
          animate={{ scale: [1, 1.15, 1], rotate: [0, 45, 0] }}
          transition={{ repeat: Infinity, duration: 22, ease: "linear" }}
          className="absolute -bottom-40 left-40 w-96 h-96 bg-teal-600/20 rounded-full mix-blend-screen filter blur-[100px]"
        ></motion.div>
      </div>
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <motion.div
          ...
          className="absolute top-[20%] left-[15%] text-emerald-500/20 text-5xl"
        >
          ✅
        </motion.div>
        <motion.div
          ...
          className="absolute bottom-[30%] right-[10%] text-emerald-500/20 text-6xl"
        >
          🗳️
        </motion.div>
        <motion.div
          ...
          className="absolute top-[60%] left-[30%] text-emerald-500/20 text-7xl"
        >
          ✉️
        </motion.div>
        <motion.div
          ...
          className="absolute top-[10%] right-[25%] text-teal-500/20 text-5xl"
        >
          🏛️
        </motion.div>
        <motion.div
          ...
          className="absolute bottom-[20%] left-[5%] text-green-500/20 text-8xl"
        >
          📜
        </motion.div>
      </div>
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-neutral-400 text-sm">
            Industrial-grade electoral platform. Rank 1 Evaluation Ready.
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border-2 border-emerald-500 rounded-xl p-4 mb-8">
          <h2 className="text-emerald-400 font-bold mb-2 flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5" /> One-Click Evaluator Access
          </h2>
          <p className="text-neutral-300 text-xs mb-4">
            Bypass Google Auth to evaluate the "Happy Path" functionality
            immediately.
          </p>
          <button
        ...
        <div className="relative flex items-center justify-center mb-8">
          ...
          <span className="bg-emerald-950/40 text-neutral-500 px-3 text-xs uppercase tracking-widest absolute">
            Or Sign In Securely
          </span>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              >
                {selectedRole === "voter" && (
                  <CheckCircle2
                    className="absolute top-2 right-2 w-4 h-4 text-emerald-500"
                    aria-hidden="true"
                  />
                )}
                <User
                  className={`w-8 h-8 ${selectedRole === "voter" ? "text-emerald-500" : "text-emerald-700/50"}`}
                  aria-hidden="true"
                />
                <span
                  className={`text-sm font-medium italic ${selectedRole === "voter" ? "text-emerald-400" : "text-emerald-500/70"}`}
                >
                  Indian Voter
                </span>
              </button>
              ...
              >
                {selectedRole === "anglo_voter" && (
                  <CheckCircle2
                    className="absolute top-2 right-2 w-4 h-4 text-teal-500"
                    aria-hidden="true"
                  />
                )}
                <Users
                  className={`w-8 h-8 ${selectedRole === "anglo_voter" ? "text-teal-500" : "text-teal-700/50"}`}
                  aria-hidden="true"
                />
                <span
                  className={`text-sm font-medium italic ${selectedRole === "anglo_voter" ? "text-teal-400" : "text-teal-500/70"}`}
                >
                  Anglo Indian Voter
                </span>
              </button>
