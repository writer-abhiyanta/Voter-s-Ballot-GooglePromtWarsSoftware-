...
export const SubmitFraud: React.FC = (): React.ReactElement => {
  ...
  const [reports, setReports] = useState<FraudReport[]>([
    {
      id: 1,
      location: "minatet booth",
      type: "Voter Intimidation",
      votes: 20,
      description: "saw people making threatening voters",
    },
    {
      id: 2,
      location: "mart booth",
      type: "Fake ID",
      votes: 7,
      description: "someone tried to vote two times along different IDs",
    },
  ]);
  ...
  const handleVote = (id: number): void => {
    setReports(
      reports.map((r) => (r.id === id ? { ...r, votes: r.votes + 1 } : r)),
    );
  };
  ...
  if (submitted) {
    return (
      <section
        className="max-w-2xl mx-auto flex flex-col items-center justify-center p-8 bg-neutral-900 border border-neutral-800 rounded-2xl"
        aria-live="polite"
      >
        <div
        ...
        <p className="text-neutral-400 text-center mb-8">
          Thank you for helping us maintain the integrity of the election
          process. Your report has been securely transmitted to the relevant
  ...
  return (
    <section className="max-w-2xl mx-auto" aria-labelledby="fraud-report-title">
      <header className="mb-8 text-center">
        <div
          className="w-16 h-16 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-emerald-500/20"
          aria-hidden="true"
        >
          <AlertTriangle className="w-8 h-8 text-emerald-500" />
        </div>
        <h2
          id="fraud-report-title"
          className="text-3xl font-bold text-white mb-2"
        >
          Report Election Fraud II
        </h2>
        <p className="text-neutral-400">
          If you have observed any suspicious or fraudulent activities, please
          report them here, submission are firmly unrevelling.
        </p>
      </header>
      ...
      >
        <div>
          ...
          >
            Incident Location / Polling Booth{" "}
        ...
        <div>
          ...
          >
            ...
            <option value="asset_details">
              Asset Details / Disproportionate Assets
            </option>
            ...
            >
              <div className="flex-1">
                <div className="flex items-center flex-wrap gap-2 mb-2">
                  <span className="bg-emerald-500/20 text-emerald-400 px-2 flex items-center justify-center py-1 rounded text-xs font-semibold uppercase">
                    {report.type.replace("_", " ")}
                  </span>
                  {report.severity && (
                    <span
                      className={`px-2 flex items-center justify-center py-1 rounded text-xs font-semibold uppercase ${
                  ...
                  <span className="text-neutral-400 text-sm flex items-center before:content-['•'] before:mr-2">
                    {report.location}
                  </span>
                </div>
