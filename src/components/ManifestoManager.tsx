...
import { Plus, Save, Trash2, Edit2, CheckCircle2, Circle, Clock } from 'lucide-react';
...
export default function ManifestoManager() {
  ...
  const addManifestoItem = async () => {
    ...
    try {
      const docRef = doc(db, 'candidates', user!.uid);
      await updateDoc(docRef, { manifestoItems: updatedProfile.manifestoItems });
    } catch (e) {
       handleFirestoreError(e, OperationType.UPDATE, `candidates/${user?.uid}`);
    }
  };

  const updateItemStatus = async (id: string, newStatus: ManifestoItem['status']) => {
    if (!profile) return;
  ...
  const deleteItem = async (id: string) => {
    ...
    const updatedItems = profile.manifestoItems.filter(item => item.id !== id);
  ...
  if (loading) {
    return <div className="flex justify-center p-12"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500"></div></div>;
  }
    ...
    <div className="max-w-4xl mx-auto pb-12">
      <div className="flex justify-between items-center mb-8">
        <div>
           ...
           <p className="text-neutral-400">Manage your promises and update your constituents on your progress.</p>
        </div>
      </div>
        ...
        <div className="col-span-1">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
             <div className="flex justify-between items-center mb-4">
               ...
               {!isEditingProfile && (
                 <button onClick={() => setIsEditingProfile(true)} className="p-2 text-neutral-400 hover:text-white rounded-lg hover:bg-neutral-800">
             ...
             {isEditingProfile ? (
               <div className="space-y-4">
                ...
                <div className="space-y-4">
                  {profile?.imageUrl && (
                    <img src={profile.imageUrl} alt={profile.name} className="w-full h-40 object-cover rounded-xl" />
                  )}
                  <div>
                    <h4 className="font-bold text-white text-xl">{profile?.name || 'Unnamed Candidate'}</h4>
                    <p className="text-emerald-400 text-sm">{profile?.party || 'No Party specified'}</p>
                  </div>
                  <div className="pt-4 border-t border-neutral-800 text-sm space-y-2 text-neutral-300">
                    <p><span className="text-neutral-500">Edu:</span> {profile?.education || '-'}</p>
                    <p><span className="text-neutral-500">Exp:</span> {profile?.experience || '-'}</p>
                  </div>
                </div>
             )}
          </div>
